...
public class CreateTeacherAccount implements ActionListener {
	Login l = new Login();
	...
	int winy = 200;
	Connection c;
	public CreateTeacherAccount(Connection c) 
	{
		this.c = c;
		window = new JFrame("Create Teacher Account");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == this.create_acc)
		{
			...
			if(!u_name.equals("") && !pass.equals("") && !pass_check.equals(""))
			{
				try {
					...
					String qry = "Select uname from teacher_account where"
								+ " uname = '" + u_name + "'";
					...
					if(usrAvail && pass.equals(pass_check))
					{
						...
						else
						{
							...
							{
								id = l.id_gen();
								qry = "Select TID from teacher_account where"
										+ " TID = '" + id + "'";
							...
							qry = "INSERT INTO TEACHER_ACCOUNT VALUES "
								+ "('"+id+"', '"+u_name+"', '"+pass+"')";
		...
		{
			LoginPageTeacher lps = new LoginPageTeacher(c);
			window.dispose();
		}
	}
}
